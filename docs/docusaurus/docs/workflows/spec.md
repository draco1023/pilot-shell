...
- Features: code review — Claude Code runs the built-in `/code-review` skill at a configurable effort (default `xhigh`, set in Console → Settings → Spec Workflow → Code Review Effort) plus an inline plan-compliance & goal-truth audit; Codex runs the native changes-review agent (optional, enabled by default)
