...
**Model:** With [Model Switching](https://pilot-shell.com/docs/features/model-routing) on (default), planning runs on the configurable **Plan Model** (Opus 4.8 default, or Fable 5) and implementation + verification on the **Execution Model** (Sonnet 5 default, or Opus 4.8) — both at 1M context, no manual `/model` step. Claude Code has no native `fableplan`, so Pilot applies a Fable plan model only during plan-mode windows and an Opus execution model only during a running `/spec` (both configured in Console → Settings → Model Switching), leaving real Opus reachable the rest of the time (and any session can still switch manually: `/model claude-opus-4-8[1m]` or `/model claude-sonnet-5`). `/spec` checks your model first and blocks with a reminder to run `/model opusplan` if you're on the wrong one (it requires Opus when Model Switching is off). With Model Switching off, **Claude Fable 5** (`/model fable`) runs the whole workflow single-model, and Pilot preserves your saved Fable model instead of overwriting it.
