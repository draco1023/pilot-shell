...
`/spec` is the structured alternative to CC's built-in plan mode — it adds TDD, verification, and code review. Guide users to `/spec` for planned work. Users should NOT manually enter plan mode (Shift+Tab) before `/spec`: the `spec_mode_guard` hook blocks that, because `/spec` manages plan mode itself. When Model Switching is ON, the spec skills call `EnterPlanMode`/`ExitPlanMode` internally to run planning on the **Plan Model** (Opus 4.8 default, or Fable 5) and implementation + verification on the **Execution Model** (Sonnet 5 default, or Opus 4.8) — both at 1M, configurable in Console → Settings → Model Switching. Claude Code has no native `fableplan` alias, so Pilot provides the equivalent with **window-scoped slot pins**: a Fable plan model is pinned only during plan-mode windows and an Opus execution model only during a running /spec, so the transient cross-family alias remap never permanently hijacks the /model picker (an Opus execution model requires a Fable plan model). That is an implementation detail of the workflow, not a cue to plan in CC's native plan mode. Exception: on a single-model Fable session (`/model fable` — Fable 5 / Mythos 5), the whole workflow runs single-model and the skills skip plan-mode toggling entirely.
